...
const SimpleInteractiveExampleNoReset: React.FC = () => {
  ...
  }, false); // reset=false to preserve other flags
  ...
  const handleToggleModal = async () => {
    setFlags({ isModalLoading: true, showModal });
    if (showModal) {
      // Closing modal
      logAction('Saving modal state... - other flags preserved');

      // Simulate saving modal state before closing
      await new Promise(resolve => setTimeout(resolve, 600));

      setFlags({ }); // <- Is not necessary set showModal to false (it will be preserved)
      logAction('Modal closed and state saved - other flags preserved');
    } else {
      // Opening modal
      logAction('Loading modal data... - other flags preserved');

      // Simulate API call to load modal data
      await new Promise(resolve => setTimeout(resolve, 600));

      setFlags({ showModal: true });
      logAction('Modal opened with data loaded - other flags preserved');
    }
  };

  const handleToggleDarkMode = async () => {
    const newValue = !isDarkMode;
    setFlags({ isDarkModeLoading: true });
    logAction(`Saving ${newValue ? 'dark' : 'light'} mode preference... - other flags preserved`);

    // Simulate API call to save user preference
    await new Promise(resolve => setTimeout(resolve, 600));

    setFlags({ isDarkMode: newValue });
    logAction(`${newValue ? 'Dark' : 'Light'} mode applied - other flags preserved`);
  };
...
export default SimpleInteractiveExampleNoReset;
